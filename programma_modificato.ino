...
void Labirinto() {
   ...
   {
     stopRuote();
     long cmSx = distanza(trigPinSx, ecoPinSx);
     long cmDx = distanza(trigPinDx, ecoPinDx);
     if ( cmSx > cmDx )
       goSinistra();
     else
       goDestra();
...
long distanza(int trigPinTemp, int echoPinTemp){
    ...
    digitalWrite(trigPinTemp, LOW);
    long duration = pulseIn(echoPinTemp, HIGH);
    long cm = 0.034 * duration / 2;
    return cm;
}
